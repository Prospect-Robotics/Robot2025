...
public class Drive extends SubsystemBase {
  ...
  public Drive(ShuffleboardTabs shuffleboard) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      shuffleboard
          .getTab("swerve")
          .addDouble(String.format("Module [%d] position", i), () -> getPosition(temp));
    }
  }
  ...
  public void drive(double xSpeed, double ySpeed, double rotation) {
    drivetrain.setControl(
        fieldCentricApplier
            ...
            .withRotationalRate(rotation)); // Note: might not work, will need testing.
  }
  ...
  StructArrayPublisher<SwerveModuleState> expectedState =
      NetworkTableInstance.getDefault()
          .getStructArrayTopic("expected state", SwerveModuleState.struct)
          .publish();
